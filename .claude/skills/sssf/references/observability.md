...
**Every payload is redacted and bounded on the way in.** `tracer.event()` runs `redact.scrub()` before either sink sees it, so `events.jsonl` and `payload_json` never hold a secret-named key's value, a credential-shaped string (`Bearer …`, `ghp_…`, `sk-…`, `AKIA…`, a JWT, a PEM block), or the value of a secret-named environment variable. Strings are capped at 4000 chars, lists at 100 items, nesting at 8 levels — one 4MB stdout no longer becomes 4MB of trace. This is pattern-based redaction, not a guarantee: trace a reference to an evidence file rather than copying a private payload into the db.
